...
class AdminGroup(app_commands.Group):
    ...
    async def upload_content(self, interaction: discord.Interaction, content_type: str):
        ...
        async with pool.acquire() as conn:
            ...
            elif content_type == "song_sections":
                with open(file_path, newline='', encoding='utf-8') as csvfile:
                    reader = csv.DictReader(csvfile)
                    for row in reader:
                        await conn.execute(
        ...
        succesfull = get_translation(language, "upload_content.succesfull", inserted=inserted, content_type=content_type)
        await interaction.response.send_message(
            succesfull,
            ephemeral=True
        )
