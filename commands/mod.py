...
class ModGroup(app_commands.Group):
    ...
    @app_commands.describe(
        agency="Agencia que recibirá los creditos",
    ...
    async def give_credits(self, interaction: discord.Interaction, agency: str, gcredits: int):
        ...
        async with pool.acquire() as conn:
            ...
            await conn.execute(
                "UPDATE users SET credits = credits + $1 WHERE agency_name = $2",
                gcredits, agency)

        await interaction.response.send_message(f"## ✅ Se entregaron 💵{format(gcredits,',')} a `{agency}`.", ephemeral=False)

    @give_credits.autocomplete("agency")
